...
This plugin adds a hash value to event as an identity. User can config `target` to change the output field
...
The value of this setting affects the _default_ value of <<plugins-{type}s-{plugin}-???>>
